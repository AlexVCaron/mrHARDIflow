...
process scil_compute_dti_fa {
    ...
    publishDir "${params.output_root}/${sid}/dti", saveAs: { f -> ("$publish" == "true") ? f.contains("metadata") ? null : f : null }, mode: params.publish_mode, overwrite: true
...
process scil_dti_and_metrics {
    ...
    publishDir "${params.output_root}/${sid}/dti", saveAs: { f.contains("metadata") ? null : f }, mode: params.publish_mode, overwrite: true
