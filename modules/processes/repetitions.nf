...
process ants_register_dwi_repetition {
    ...
    publishDir "${params.output_root}/all/${sid}/$caller_name/${task.process.replaceAll(":", "/")}", mode: "link", enabled: params.publish_all, overwrite: true
    ...
    mrhardi b0 extract --in $dwi --bvals $bval --out rep_b0 --config $b0_config
    mrhardi ants_registration --target ${target_b0} --moving rep_b0.nii.gz --out b0_registration --config $reg_config
    mrhardi ants_transform --in $dwi --ref ${target_b0} --trans b0_registration0GenericAffine.mat --bvecs $bvec --out ${dwi.simpleName}__rep_registered --config $trans_config
...
process ants_register_t1_repetition {
    ...
    script:
        def command = ""
        if (!mask.empty()) {
            command += "mrhardi ants_transform --in $mask --ref $ref_t1 --trans t1_registration0GenericAffine.mat --out ${mask.simpleName}__rep_registered"
        }
        """
        export OMP_NUM_THREADS=$task.cpus
        export ITK_GLOBAL_DEFAULT_NUMBER_OF_THREADS=$task.cpus
        export OPENBLAS_NUM_THREADS=1
        mrhardi ants_registration --target $ref_t1 --moving $t1 --out t1_registration --config $reg_config
        mrhardi ants_transform --in $t1 --ref $ref_t1 --trans t1_registration0GenericAffine.mat --out ${t1.simpleName}__rep_registered
        $command
        """
}